 ...
 */
template <int BLOCK_SIZE> __global__ void MatrixMulCUDA(float *C, const float *A,
...
template <typename T>
void computeMM(const T *A, const T *B, T *C, int m, int n, int k)
{
    return;
}
...
void computeMM(const float *A, const float *B, float *C, int m, int n, int k)
{
    ...
    MatrixMulCUDA<32><<<grid, threads>>>(C, A, B, k, k);
